package com.project.parkinglot.service.vehicle.sedan;
...
public class SedanVehicleService implements VehicleService {
	...
	private final VehicleRepository sedanRepository;
	private final ParkinglotService parkinglotService;


	public void checkIn(Vehicle vehicle) {
		Boolean existedVehicle = sedanRepository.existsByLicensePlate(vehicle.getLicensePlate());
		if(existedVehicle)
		...
		sedanRepository.save(vehicleEntity);
	}


	public Vehicle getVehicle(String id) {
		return Vehicle.toModel(sedanRepository.findByLicensePlate(id)
	...
	public void checkOut(CheckOutModel checkOutModel) {
		Vehicle vehicleExisted = Vehicle.toModel(sedanRepository.findByLicensePlate(checkOutModel.getLicensePlate())
				.orElseThrow(()-> new DataNotFoundException(ExceptionType.LICENSEPLATE_NOT_FOUND)));
		...
		if(checkOutModel.getHoursToStayed()<3)
			...
			parkinglot.setTotalAmount(parkinglot.getTotalAmount()+priceList.getTwoHourToFourHour());
		else if(checkOutModel.getHoursToStayed()>=5 && checkOutModel.getHoursToStayed()<9)
			parkinglot.setTotalAmount(parkinglot.getTotalAmount()+priceList.getFourHourToEightHour());
		else if(checkOutModel.getHoursToStayed()>=9 && checkOutModel.getHoursToStayed()<15)
			parkinglot.setTotalAmount(parkinglot.getTotalAmount()+priceList.getEightHourToFourTeenHour());
		else if(checkOutModel.getHoursToStayed()>=15 && checkOutModel.getHoursToStayed()<=24)
			parkinglot.setTotalAmount(parkinglot.getTotalAmount()+priceList.getFourTeenHourToTwenyFourHour());
		parkinglot.setCurrentCapacity(parkinglot.getCurrentCapacity()+1);
		parkinglotService.updateParkinglot( parkinglot, parkinglot.getId());
		sedanRepository.deleteById(vehicleExisted.getId());
	}
